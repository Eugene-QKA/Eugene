...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $stmt = $conn->prepare("SELECT id, name, password, role FROM users WHERE email = ? AND role = 'collector'");
  ...
  if ($stmt->num_rows > 0) {
    ...
    if (password_verify($password, $hashed_password)) {
      ...
      $_SESSION['collector_name'] = $name;
      $_SESSION['collector_role'] = $role;
    ...
    $error = "No collector found with that email.";
  }
